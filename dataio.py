...
class Plate(Dataset):
    def __init__(self, sidelength, load_coords=[0., 0., 0.],
                 pretrain=False):
        super().__init__()
        torch.manual_seed(0)
        ...
        self.W = 10
        self.H = 10
        self.load_coords = torch.tensor(load_coords).view(-1, 3)
    ...
    def __getitem__(self, idx):
        start_time = self.load_coords[0, 0]  # time to apply  initial conditions

        x = torch.rand(self.sidelength ** 2, 1) * 10
        y = torch.rand(self.sidelength ** 2, 1) * 10
        ...
        in_val = np.sin(np.pi / W * coords[:,1]) * np.sin(np.pi / H * coords[:,2])
        in_val = torch.tensor(in_val)
        # only enforce initial conditions around start_time
        in_val = torch.where((coords[:, 0, None] == start_time), in_val, torch.Tensor([0]))
        dirichlet_mask = (coords[:, 0, None] == start_time)

        self.counter += 1

        return {'coords': coords}, {'initial_values': in_val, 'dirichlet_mask': dirichlet_mask}
