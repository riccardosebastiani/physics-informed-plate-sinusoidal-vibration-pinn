...
def plate_loss(model_output, gt):
    initial_values = gt['initial_values']
    ...
    om = 2 * np.pi
    h = torch.tensor(0.2)
    rho = torch.tensor(2750)
    D = torch.tensor(30000)

    load = np.sin(np.pi /W * x[:,:,1])*np.sin(np.pi /H * x[:,:,2])*np.cos(om * x[:,:,0])
    load = torch.tensor(load)
    du, status = diff_operators.jacobian(y, x)
    dudt = du[..., 0]
    du = du[...,0,:] #shape (meta_batch_size, num_points, 3) ----> dudt, dudx, dudy
    ddu, status = diff_operators.jacobian(du, x)
    dudt2 = ddu[...,0,0, None] #shape (meta_batch_size, num_points, 1)
    dudx2 = ddu[...,1,1] #shape (meta_batch_size, num_points)
    dudy2 = ddu[...,2,2] #shape (meta_batch_size, num_points)
    ddu = ddu[...,0:,0:] #shape (meta_batch_size, num_points,2 ,2)
    dddu, status = diff_operators.jacobian(ddu, x[:,:,0:])
    dudx3 = dddu[...,0,0,0]
    dudy3 = dddu[...,1,1,1]
    dudxxy = dddu[...,0,0,1] #shape (meta_batch_size, num_points)
    ddddu, status = diff_operators.jacobian(dddu, x[:,:,0:])
    dudx4 = dddu[..., 0, 0, 0, 0]
    dudy4 = dddu[..., 1, 1, 1, 1]
    dudxxyy = dddu[..., 0, 0, 1, 1]  # shape (meta_batch_size, num_points)

    lap = dudx4[...,None] + dudy4[...,None] + 2*dudxxyy[...,None]

    diff_constraint_hom = rho * h* dudt2 + lap - load/D

    dirichlet = y[dirichlet_mask] - initial_values[dirichlet_mask]
